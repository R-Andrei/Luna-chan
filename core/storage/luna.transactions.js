...
class StorageWorker {
    constructor(user, token, address, database, collections) {
        ...
        this.client = new mongodb_1.default.MongoClient(`mongodb+srv://${user}:${token}@${address}/${database}/?retryWrites=true&w=majority`, { useNewUrlParser: true });
    }
    ...
    async update_server(server, record_type) {
        ...
        return new Promise((resolve, reject) => {
            ...
            collection.findOneAndUpdate({ "server_id": server.id, "type": record_type }, data_set, { upsert: true })
                .then((_result) => resolve(`Transaction finished. Inserted data set (${Object.values(data_set['$set']).join(', ')}) into collection ${this.collections.servers}'.`))
                .catch((err) => reject(err));
        });
    }
